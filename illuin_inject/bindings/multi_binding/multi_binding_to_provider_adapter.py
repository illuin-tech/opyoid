...
from .item_binding import EMPTY, ItemBinding
...
class MultiBindingToProviderAdapter(BindingToProviderAdapter[MultiBinding]):
    ...
    def create(self, binding: MultiBinding[InjectedT], provider_creator: "ProviderCreator") -> Provider[InjectedT]:
        unscoped_provider = ListProvider(
            [
                self._get_item_provider(binding, item_binding, provider_creator)
    ...
    def _get_item_provider(self,
                           ...
                           provider_creator: "ProviderCreator") -> Provider[InjectedT]:
        if item_binding.bound_instance is not EMPTY:
            item_binding = InstanceBinding(
                parent_binding.target_type,
                item_binding.bound_instance,
                parent_binding.annotation,
            )
        elif item_binding.bound_type is not EMPTY:
        ...
        return self._item_provider_factory.create_from_binding(item_binding, provider_creator)
