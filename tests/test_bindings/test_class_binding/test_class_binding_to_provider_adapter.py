...
class TestClassBindingToProviderAdapter(unittest.TestCase):
    ...
    def test_create_provider_without_args(self):
        self.providers_creator.get_providers.return_value = [self.mock_scope_provider]
    ...
    def test_create_scoped_provider(self):
        ...
        self.providers_creator.get_providers.return_value = [mock_scope_provider]
        provider = self.adapter.create(ClassBinding(MyType, scope=ThreadScope), self.providers_creator)

        self.providers_creator.get_providers.assert_called_once_with(Target(ThreadScope))
    ...
    def test_non_injectable_scope_raises_exception(self):
        ...
        with self.assertRaises(NonInjectableTypeError):
            self.adapter.create(ClassBinding(MyType), self.providers_creator)
